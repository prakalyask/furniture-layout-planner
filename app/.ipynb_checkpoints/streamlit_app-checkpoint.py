    ...
    if st.sidebar.button("Use Recommended Furniture"):
        ...
        for furniture_name in preset_furniture:
            ...
            if furniture_name == "Dining Chair":
                ...
                factory_method = getattr(FurnitureFactory, f"create_{options['type']}")
    ...
    if generate_btn or (st.session_state.room is not None and st.session_state.optimization_complete):
        if generate_btn:
            ...
            if add_door:
                if door_position == "Bottom Wall":
                    door = RoomFeature('door', room_width/2 - door_width/2, 0, door_width, 0.5, 0)
                elif door_position == "Top Wall":
                    door = RoomFeature('door', room_width/2 - door_width/2, room_height - 0.5, door_width, 0.5, 0)
                elif door_position == "Left Wall":
                    door = RoomFeature('door', 0, room_height/2 - door_width/2, 0.5, door_width, 0)
                else:  # Right Wall
                    door = RoomFeature('door', room_width - 0.5, room_height/2 - door_width/2, 0.5, door_width, 0)
            ...
            if add_window:
                if window_position == "Bottom Wall":
                    window = RoomFeature('window', room_width/2 - window_size/2, 0, window_size, 0.5, 0)
                elif window_position == "Top Wall":
                    window = RoomFeature('window', room_width/2 - window_size/2, room_height - 0.5, window_size, 0.5, 0)
                elif window_position == "Left Wall":
                    window = RoomFeature('window', 0, room_height/2 - window_size/2, 0.5, window_size, 0)
                else:  # Right Wall
                    window = RoomFeature('window', room_width - 0.5, room_height/2 - window_size/2, 0.5, window_size, 0)
            ...
            if selected_furniture:
                ...
                if not furniture_list:
                    ...
                    furniture_list = optimizer.recommend_furniture(room_size_category)
            ...
            # Generate layouts
            layouts = optimizer.create_alternative_layouts(num_layouts, furniture_list)
        ...
        with col1:
            if st.button("Previous Layout", disabled=current_idx == 0):
                ...
                st.session_state.current_layout_idx = (current_idx - 1) % len(st.session_state.layouts)
        ...
        with col2:
            st.write(f"Layout {current_idx + 1} of {len(st.session_state.layouts)}")
        ...
        # Show empty room
        fig, ax = plt.subplots(figsize=(10, 8))
        ax.add_patch(plt.Rectangle((0, 0), room_width, room_height, fill=False, edgecolor='black'))
        ...
        st.info("Configure your room and furniture in the sidebar, then click 'Generate Layouts'.")
...
with details_col:
    if st.session_state.optimization_complete:
        ...
        if st.session_state.metrics:
            ...
            with col1:
                st.metric("Space Efficiency", f"{metrics['space_efficiency']:.2f}")
                st.metric("Zone Coherence", f"{metrics['zone_coherence']:.2f}")
                st.metric("Clearance Score", f"{metrics['clearance_score']:.2f}")
                
            with col2:
                st.metric("Relationship Score", f"{metrics['relationship_score']:.2f}")
                st.metric("Pathway Quality", f"{metrics['pathway_quality']:.2f}")
                st.metric("Aesthetics Score", f"{metrics['aesthetics_score']:.2f}")
                
        # Download button
        st.download_button(
            label="Download Layout as Image",
            data=open(f"output/optimized_layout.png", "rb").read() if os.path.exists("output/optimized_layout.png") else None,
            file_name=f"furniture_layout_{st.session_state.current_layout_idx + 1}.png",
            mime="image/png",
            disabled=not os.path.exists("output/optimized_layout.png")
        )
...
# Footer
st.markdown("---")
st.caption("AI Furniture Layout Planner | Created with Streamlit and Reinforcement Learning")